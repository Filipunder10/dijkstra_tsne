...
def dijkstra(csgraph, directed=True, indices=None,
             ...
             bint min_only=False, sample_indices=None, k=-1):
    """
    ...
    # initialize/validate sample indices
    if sample_indices is not None:
        sample_indices = np.array(sample_indices, order='C', dtype=ITYPE, copy=True)
        sample_indices = np.atleast_1d(sample_indices).reshape(-1)
        sample_indices[sample_indices < 0] += N
        if np.any(sample_indices < 0) or np.any(sample_indices >= N):
            raise ValueError("sample indices out of range 0...N")
    ...
    if directed:
        ...
        if min_only:
            ...
            _dijkstra_undirected_early(indices,
                                 csr_data, csgraph.indices, csgraph.indptr,
...
@cython.boundscheck(False)
cdef int _dijkstra_undirected_early(
            int[:] source_indices,
            double[:] csr_weights,
            int[:] csr_indices,
            int[:] csr_indptr,
            ...
            DTYPE_t limit,
            int[:] sample_indices,
            int k_nearest) except -1:
    cdef:
        unsigned int Nind = dist_matrix.shape[0]
        unsigned int N = dist_matrix.shape[1]
        unsigned int i, k, j_source, j
        unsigned int sample_nodes_visited = 0
        int return_pred = (pred.size > 0)
        FibonacciHeap heap
        FibonacciNode *v
        FibonacciNode* nodes = <FibonacciNode*> malloc(N *
    ...
    cdef unsigned char[::1] visited_flags = np.zeros(N, dtype=np.uint8)
    for j in range(sample_indices.shape[0]):
        visited_flags[sample_indices[j]] = 1
    ...
    for i in range(Nind):
        ...
        sample_nodes_visited = 0

        while heap.min_node:
            v = remove_min(&heap)
            v.state = SCANNED

            if visited_flags[v.index] == 1:
                sample_nodes_visited += 1
                if sample_nodes_visited > k_nearest:
                    break
